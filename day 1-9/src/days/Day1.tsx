...
export const Day1 = () => {
  ...
  const calculate = () => {
    ...
    lines.forEach((line, index) => {
      const found = line.match(DIGITREGEX);
      //flip it and reverse it #performanceleftthechat
      const foundReverse = reverseString(line).match(DIGITREGEXREVERSE);
      console.log(foundReverse);
      if (
        found &&
        found.length > 0 &&
        foundReverse &&
        foundReverse.length > 0
      ) {
        //combine first digit and last digit and add to sum
        const number = parseInt(
          `${getNumber(found[0])}${getNumber(reverseString(foundReverse[0]))}`
        );
        if (isNaN(number) || number > 99) console.log("ERROR", number);
        console.log(`number for line: ${index} is ${number}`);
  ...
  return (
    <div>
      <button onClick={calculate}>Calculate day 1</button>
