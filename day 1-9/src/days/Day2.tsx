...
import { getNumber } from "../utility.ts";
...
function getMaxNumberForAoC2(line: string, match: any): number {
  const redFound = line.match(match);
  let max = 0;
  if (redFound) {
    redFound.forEach((num) => {
      const red = getNumber(num);
      if (max < red) max = red;
    });
  }
  return max;
...
function getGameNumber(line: string): number {
  const found = line.match(/Game (\d+):/);
  return found ? parseInt(found[found.length - 1]) : -1;
}

function isGameValid(line: string): boolean {
  const maxRed = getMaxNumberForAoC2(line, /(\d+) red/g);
  const maxGreen = getMaxNumberForAoC2(line, /(\d+) green/g);
  const maxBlue = getMaxNumberForAoC2(line, /(\d+) blue/g);
  console.log(maxRed, maxGreen, maxBlue);

  return maxRed > 12 || maxBlue > 14 || maxGreen > 13 ? false : true;
}

function powerofGame(line: string): number {
  const maxRed = getMaxNumberForAoC2(line, /(\d+) red/g);
  const maxGreen = getMaxNumberForAoC2(line, /(\d+) green/g);
  const maxBlue = getMaxNumberForAoC2(line, /(\d+) blue/g);
  console.log(maxRed, maxGreen, maxBlue);
...
export const Day2 = () => {
  ...
  const calculatePartOne = () => {
    ...
    lines.forEach((line) => {
      ...
      sum += isGameValid(line) ? gameNumber : 0;
    });
    setValue(sum);
  };
  ...
  const calculatePartTwo = () => {
    ...
    lines.forEach((line, index) => {
      const gameNumber = getGameNumber(line);
      console.log("gamenumber", gameNumber);
      console.log("power", powerofGame(line));
  ...
  return (
    <div>
      <div>
        <button onClick={calculatePartOne}>Calculate day 2 Part 1</button>
        <div>Your value is {value}</div>
      </div>
      <div>
        <button onClick={calculatePartTwo}>Calculate day 2 Part 2</button>
        <div>Your value is {part2}</div>
      </div>
    </div>
  );
};
