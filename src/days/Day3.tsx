...
const DATA = `.......................153..988....502..842.........588.....441.468......481..........314...715.57............................163..992..512.
............805............*......#.............%...............*........=......%......................#......*.............-....#....*.....
...
function get2DArrayFromData() {
  return DATA.split("\n").map((row) => row.split(""));
}

function getEmpty2DArrayFromData(rowLength, columnLength) {
  return Array.apply(null, Array(rowLength)).map((a) => {
    return Array.apply(null, Array(columnLength)).map((a) => "0");
  });
}

function IsStringSpecialCharacter(column: string) {
  if (
    column === "." ||
    column === "1" ||
    column === "2" ||
...
const NUMBERREGEX: any = /(\d)/g;
function IsStringNumber(column: string) {
  if (
    column === "1" ||
    column === "2" ||
    column === "3" ||
...
function checkIfIndexExists(
  ...
  if (
    rowIndex < engineMap.length &&
    rowIndex >= 0 &&
    columnIndex < engineMap[rowIndex].length &&
    columnIndex >= 0
  ) {
    return true;
  }
...
function checkIfNumberAndMarkUsed(
  engineMap: string[][],
  rowIndex: number,
  columnIndex: number,
  numbersUsed: string[][]
) {
  if (checkIfIndexExists(engineMap, rowIndex, columnIndex)) {
    if (IsStringNumber(engineMap[rowIndex][columnIndex])) {
      if (numbersUsed[rowIndex][columnIndex] === "1") {
        return ".";
      }
      numbersUsed[rowIndex][columnIndex] = "1";
      return parseInt(engineMap[rowIndex][columnIndex]);
    }
    return ".";
  }
  return ".";
...
function checkAndExtractNumber(
  engineMap: string[][],
  rowIndex: number,
  columnIndex: number,
  numbersUsed: string[][]
) {
  //to lazy just assume there are a max of 4 digits
  let curlength = 0;
  let exit = false;
  let usedFound = false;
  let numberString = "";
  do {
    //check first if number was already processed
    if (checkIfIndexExists(numbersUsed, rowIndex, columnIndex + curlength)) {
      if (numbersUsed[rowIndex][columnIndex + curlength] === "1") {
    ...
    //check for number
    const digit = checkIfNumberAndMarkUsed(
      engineMap,
      rowIndex,
      columnIndex + curlength,
      numbersUsed
    );
    if (digit !== ".") {
      curlength++;
  ...
  } while (curlength < 4 && !exit);

  //only continue left if a number was found, as 0 index is omitted on left check
  if (numberString.length > 0 && !usedFound) {
    curlength = 1;
    exit = false;
    do {
      //check first if number was already processed
  ...
  console.log(`numberString for ${rowIndex}${columnIndex}: `, numberString);
  return numberString.length > 0 ? parseInt(numberString) : 0;
}
...
export const Day3 = () => {
  ...
  const calculate = () => {
    const engineMap = get2DArrayFromData();
    const numbersUsed = getEmpty2DArrayFromData(
      engineMap.length,
      engineMap[0].length
    );
    let sum = 0;
    engineMap.forEach((row, rowIndex) => {
      row.forEach((column, columnIndex) => {
        if (IsStringSpecialCharacter(column)) {
          console.log(
            `found special character at ${rowIndex}|${columnIndex} ${column}`
          );
          //search all adjavent field for numbers

          //search left and right
          sum += checkAndExtractNumber(
            engineMap,
            rowIndex,
            columnIndex - 1,
            numbersUsed
    ...
    engineMap.forEach((row, rowIndex) => {
      row.forEach((column, columnIndex) => {
        if (column == "*") {
          ...
          //search left and right
          const leftValue = checkAndExtractNumber(
            engineMap,
            rowIndex,
            columnIndex - 1,
            numbersUsed
          );
          const rightValue = checkAndExtractNumber(
            engineMap,
            rowIndex,
            columnIndex + 1,
            numbersUsed
          ...
          //   //search top
          const topLeft = checkAndExtractNumber(
            engineMap,
            rowIndex - 1,
            columnIndex - 1,
            numbersUsed
          );
          const topMiddle= checkAndExtractNumber(
            engineMap,
            rowIndex - 1,
            columnIndex,
            numbersUsed
          );
          const topRight = checkAndExtractNumber(
            engineMap,
            rowIndex - 1,
            columnIndex + 1,
            numbersUsed
          ...
          //search bottom
          const bottomLeft = checkAndExtractNumber(
            engineMap,
            rowIndex + 1,
            columnIndex - 1,
            numbersUsed
          );
          const bottomMiddle = checkAndExtractNumber(
            engineMap,
            rowIndex + 1,
            columnIndex,
            numbersUsed
          );
          const bottomRight = checkAndExtractNumber(
            engineMap,
            rowIndex + 1,
            columnIndex + 1,
            numbersUsed
          ...
          const result = [leftValue, rightValue, topLeft, topMiddle, topRight, bottomLeft, bottomMiddle, bottomRight].filter(x => x > 0);
          if(result.length > 1){
            const gearproduct = result.reduce((a,b) => a*b);
            console.log("found gear:", gearproduct);
            sum += gearproduct;
          }
        }
      });
